...
# ╔═╡ fd5c1d40-35e8-11ec-1df6-4997fe05bb3f
begin
    # using Pkg; Pkg.activate()
    using OrdinaryDiffEq, ModelingToolkit, DiffEqOperators, DomainSets, CairoMakie,ColorSchemes, DataFrames, CSV, Statistics, Optim, Polynomials, PlutoUI, DifferentialEquations, SpecialFunctions, JLD2, XLSX
end

# ╔═╡ f6964597-7821-49cd-9397-b008c7fcbf44
set_theme!(theme_light()); update_theme!(fontsize=22, linewidth=3)

# ╔═╡ 30e6bd09-77af-43ec-a8cf-2070386d7f87
md"# code to solve PDE
* diffusion equation
* Reed-Ehrlich model for concentration-dependent diffusion
"

# ╔═╡ 0342cd47-1d90-4955-9e7c-5d0cc8e5d129
# Reed-Ehrlich model in Reed, Erlich:
#  SURFACE DIFFUSION, ATOMIC JUMP  RATES AND  THERMODYNAMICS
struct DiffusionCoeff
	# diffusion coeff. at dilute conditions
	D₀::Float64 # nm² / s
	# parameter characterizing gas-gas attractions (η > 1 for attractions)
	η::Float64 
end

# ╔═╡ cbe16bb0-70af-4d5d-aa30-6aaa3ee55648
mutable struct SURMOF_Gas_System
	# thickness of the SURMOF
	ℓ::Float64 # nm
	# diffusion coefficient of gas in the SURMOF
	#  D = D(c)
	D::DiffusionCoeff
	# surface resistance
	R::Float64
	# equilibrium concentration in the film
	cₑ::Float64 # g / nm³
	# max concentration (for computing fractional loading, θ)
	c_max::Float64 # g / nm³
end

# ╔═╡ 23975897-4ad0-45d3-8f3e-c6a813758b9d
mutable struct PDE_Solver_Settings
	# final time to solve
	t_final::Float64 # s
	# spacial discretization step size
	Δx::Float64 # nm
	# number of spatial discretation points including boundaries
	Nₓ::Union{Int, Nothing}
end

# ╔═╡ 196462e4-c4f4-44a1-a886-5256e7e40fe1
function solve_PDE(sensor::SURMOF_Gas_System, 
				   settings::PDE_Solver_Settings; 
				   saveat=1.0, verbose=true)
	if verbose
		println("saving every ...", saveat)
	end
	# define independent variables (space-time)
	@parameters x t # x is depth into SURMOF film

	# define dependent variable (concentration)
	@variables c(..)
	...
	# Reed-Ehrlich model for D = D(c)
	θ = c(x, t) / sensor.c_max
	β = sqrt(1 - 4 * θ * (1 - θ) * (1 - sensor.D.η))
	ϵ = (β - 1 + 2 * θ) / (2 * sensor.D.η * (1 - θ))
	D = sensor.D.D₀ * (1 + ϵ) ^ 3 / (1 - θ) / (1 + sensor.D.η * ϵ) ^ 4 * (1 + 2 * (1 - β) / β)
	
	# diffusion equation
	diff_eq = expand_derivatives(∂t(c(x, t)) ~ ∂x(D * ∂x(c(x, t))))
	
	# # temporal boundary conditions = initial condition 
	# #    (empty of gas at t=0)
	ic = c(x, 0) ~ 0.0

	# # spatial boundary conditions
	# #    since x is depth, x = 0 => MOF-air interface
	# #	                   x = ℓ => MOF-gold interface
	# bc_mof_gas  = c(0.0, t) ~ sensor.cₑ      # Dirichlet
	# Robin BC
	θ_boundary = c(0.0, t) / sensor.c_max
	β_boundary = sqrt(1 - 4 * θ_boundary * (1 - θ_boundary) * (1 - sensor.D.η))
	ϵ_boundary = (β_boundary - 1 + 2 * θ_boundary) / (2 * sensor.D.η * (1 - θ_boundary))
	D_boundary = sensor.D.D₀ * (1 + ϵ_boundary) ^ 3 / (1 - θ_boundary) / (1 + sensor.D.η * ϵ_boundary) ^ 4 * (1 + 2 * (1 - β_boundary) / β_boundary)
	bc_mof_gas = D_boundary  * ∂x(c(0.0, t)) - sensor.R * (c(0.0, t) - sensor.cₑ) ~ 0.0 # Robin, erf
	# # bc_mof_gas = ∂x(c(0.0, t)) / (1 + exp(sensor.D.α * (c(0.0, t) - sensor.D.χ))) * sensor.R * sensor.D.D₀ + sensor.cₑ - c(0.0, t) ~ 0.0 # Robin, sigmoid
	
	bc_mof_gold = ∂x(c(sensor.ℓ, t)) ~ 0.0 # no flux
	
	# spacetime boundary conditions
	spacetime_bcs = [ic, bc_mof_gas, bc_mof_gold]

	# define space-time plane
	domains = [x ∈ Interval(0.0, sensor.ℓ), t ∈ Interval(0.0, settings.t_final)]

	# put it all together into a PDE system
	@named pdesys = PDESystem(diff_eq, spacetime_bcs, domains, [x, t], [c(x, t)]);

	# discretize space
	discretization = MOLFiniteDifference([x=>settings.Δx], t)
	...
	settings.Nₓ = length(prob.u0) + 1 # change to +2 for Dirichlet
	if sensor.R == 0.0
		settings.Nₓ += 1
	end
	if verbose
		println("# spatial steps = ", settings.Nₓ)
	end
	@assert (length(0:settings.Δx:sensor.ℓ) == settings.Nₓ)
	...
	# handle boundary conditions since they are not included
	@variables c[1:settings.Nₓ](t)
	data[:, "c[$(settings.Nₓ)](t)"] = sol[c[settings.Nₓ]]
	if ! ("c[1](t)" in names(data))
		data[:, "c[1](t)"] = sol[c[1]]
	end
	
	# re-order columns since sometimes c[1](t) at the end.
	ordered_cols = ["c[$i](t)" for i = 1:settings.Nₓ]
	pushfirst!(ordered_cols, "m")
	pushfirst!(ordered_cols, "time")
	rename!(data, Dict(:timestamp => "time"))

	###
	#   compute m(t), mass adsorbed in thin film
	#   by numerically integrating c(x, t) over x.
	###
	# insert a new column for the mass
	if ! ("m" in names(data))
		insertcols!(data, 2, "m" => zeros(nrow(data)))
	end

	# loop over rows = time
	for k = 1:nrow(data)
		# trapezoid rule: https://en.wikipedia.org/wiki/Trapezoidal_rule
		m = 0.0 # initialize mass at this time
		for i = 2:settings.Nₓ
			m += data[k, "c[$(i-1)](t)"] + data[k, "c[$i](t)"]
		end
		data[k, :m] = m * settings.Δx / 2 # ng/cm²
	end
	
	return data[:, ordered_cols]
end

# ╔═╡ 528696aa-7f45-4f60-8262-d35c6a18aa72
# Reed-Ehrlich model for D = D(c) over concentration range seen
function viz_D(sensor::SURMOF_Gas_System)
	fig = Figure()
	ax = Axis(fig[1, 1], xlabel="c", ylabel="D")
	
	c = range(0.0, sensor.cₑ, length=100)
...
# ╔═╡ b14161da-0233-444d-adaa-9dab5e7474b5
md"# fit PDE to dynamic QCM-MOF response curve"

# ╔═╡ 819e70b2-c332-4e6c-a966-433188249b77
concentrations = ["1 ppm", "2 ppm", "5 ppm", "10 ppm", "50 ppm", "500 ppm", 
				  "1000 ppm"]

# ╔═╡ b9c6d8db-7444-48eb-a424-ae9eaa0c5059
gases = ["o-xylene", "p-xylene", "m-xylene"]

# ╔═╡ ca05cb06-2953-4834-8e85-f1030a3e5d86
begin
	_data = Dict()
	for gas in gases
		_data[gas] = Dict()
		for concentration in concentrations
			_data[gas][concentration] = Dict()
...
# ╔═╡ bae0316a-65ee-41ea-b0f7-684d2ebda881
@bind mof Select(["HKUST1", "CuBDC", "CuBPDC",	"UiO66", "UiO67", "UiO68-NH2"])

# ╔═╡ 66491318-7432-48e0-9b8c-1bdb469d4267
begin
	# load in processed data
	
	data = _data[gas]["5 ppm"][mof]
end

# ╔═╡ 4295abbb-75a8-41de-b1ab-9543eab1b3e3
begin
	# thickness
	ℓ     = load("thickness_c_max.jld2")["ℓ"]     # nm
	# max concentration for fractional loading
	c_max = load("thickness_c_max.jld2")["c_max"][gas] # ng / (cm² * nm)
end

# ╔═╡ 11905fca-39d7-4ea0-8150-059586526df1
function viz_PDE_soln_with_data(pde_sol; zoom=false)
	fig = Figure()
	ax = Axis(fig[1, 1], 
		      xlabel="time, t [s]", 
		      ylabel="mass, m(t) [ng/cm²]", 
		      title="$(gas) $(concentration)")

	# plot experimental data
	ids_pos = data[:, "time/s"] .>= 0
	scatter!(data[ids_pos, "time/s"], data[ids_pos, "mass/(ng/cm²)"], 
		     label="data")
	# plot PDE model
	lines!(pde_sol[:, "time"], pde_sol[:, "m"], 
...
# ╔═╡ d14115a1-ef53-4fbf-8ee6-0d0817f22ee1
begin
	D = DiffusionCoeff(450.0, 1.75) # D₀, η
	
	sensor = SURMOF_Gas_System(ℓ, D, 1.0, maximum(data[:, "mass/(ng/cm²)"]) / ℓ, c_max) # ℓ, D, R, cₑ, c_max

	# warning: will take a long time if spatial step small
	settings = PDE_Solver_Settings(data[end, "time/s"], 5.0, nothing)
	
	pde_sol = solve_PDE(sensor, settings, saveat=5.0)
end

# ╔═╡ bcb35d3d-892b-4572-9b34-d7f2df0cd6de
viz_PDE_soln_with_data(pde_sol, zoom=true)

# ╔═╡ 66173061-8d5e-4f41-beb0-cc4ff96050f4
viz_D(sensor)

# ╔═╡ 54ca5511-36ec-4273-ad08-51bf2d2e9c7f
begin
	# manually discretize the x-domain
	#  TODO figure out how to query this from the pdesys 
	xs = collect(0:settings.Δx:sensor.ℓ)

	# create matrix of concentrations on spacetime.
	# rows = time
	# cols = space
	space_cols = ["c[$i](t)" for i = 1:settings.Nₓ]
	cs = Matrix(pde_sol[:, space_cols])
end

# ╔═╡ bfd35895-ee03-4ebb-97ad-328655ad12c2
begin
	fig_c_of_xt = Figure()
	ax = Axis(fig_c_of_xt[1, 1], 
			  xlabel="time, t [s]", 
			  ylabel="depth, x [nm]", 
			  title ="spatiotemporal concentration of gas in the SURMOF") 
	heatmap_cmap = reverse(ColorSchemes.sunset)
	hm = heatmap!(pde_sol[:, "time"], xs, cs, colormap=heatmap_cmap)
	Colorbar(fig_c_of_xt[1, 2], hm,
			 label="concentration, c(x, t) [ng/nm³]")
	fig_c_of_xt
end

# ╔═╡ 305f60b8-f814-408a-a92e-0bf9d8667550
begin
	# analytical solution with Dirichlet BC
	function m(t::Float64, sensor::SURMOF_Gas_System, N::Int=1000)
		λ(n::Int64) = π * (2 * n + 1) / (2 * sensor.ℓ)
		
		sumterm = 0
		for n in 0:N
			λ_n = λ(n)
			sumterm += exp(-λ_n ^ 2 * sensor.D.D₀ * t) / λ_n ^ 2
		end

		return sensor.cₑ * (sensor.ℓ - 2 * sumterm / sensor.ℓ)

	end
end

# ╔═╡ 688f4845-d4f1-46b1-adb5-5795c111ede8
begin	
	fig_mass = Figure()
	mass_ax = Axis(fig_mass[1, 1], 
				   xlabel = "time, t [s]", 
				   ylabel = "mass, m(t) [ng]", 
				   title = "QCM-SURMOF dynamic response")
	xlims!(0, nothing)
	ylims!(0, nothing)
	scatter!(pde_sol[:, "time"], pde_sol[:, :m], 
			 label="PDE solver")
	ts_fine = range(0.0, settings.t_final, length=150)
	lines!(ts_fine, [m(tᵢ, sensor) for tᵢ in ts_fine], 
		   label="analytical soln", color="green")
	axislegend(position=:rb)
	fig_mass
end

# ╔═╡ 6cb0767b-5e02-4711-bf2b-cf96a2843a80
function loss(D₀_α_R)
	cₑ = 4600/ℓ
	sensor = SURMOF_Gas_System(ℓ=ℓ, D₀=D₀_α_R[1], α=D₀_α_R[2], R=D₀_α_R[3], cₑ=cₑ)
	settings = PDE_Solver_Settings(maximum(uptake_data[:, "time/s"]), 5.0, nothing)
	
	uptake_data_t_nonnegative = filter(row -> row["time/s"] >= 0, uptake_data)
	
	data_pde = solve_PDE(sensor, settings, saveat=uptake_data_t_nonnegative[:, 	"time/s"])
	
	@assert(isapprox(uptake_data_t_nonnegative[:, "time/s"], data_pde[:, "time"]))
	
	return sum((data_pde[:, "m"] .- uptake_data_t_nonnegative[:, "mass/(ng/cm²)"]) .^ 2)

end

# ╔═╡ 9f37e22d-feef-406c-9891-e2113f2cad6a
with_terminal() do
	D₀_α_χ_R = [700.0, -10.0, 0.1]
	loss(D₀_α_R)
end

# ╔═╡ 76b4576f-6c9c-45ab-bb07-6ffdc4b15ae4
with_terminal() do
	D₀_α_R = [700.0, -10.0, 0.1]
	# opt_params = optimize(loss, D₀_α_R)
end

# ╔═╡ 83f93745-daeb-406a-86ec-7cbeb94a80d5
viz(D_opt)
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "3d4d675d5d3215f280f497d07f66ff63a1249a53"
...
git-tree-sha1 = "7687f6a6f6214df20b201680bf97ee35bb524e86"
